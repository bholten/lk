...
/* Transaction identity (editor multi-cursor stage E, but generic):
 * every recorded transaction is stamped with a monotonically
 * increasing serial (starting at 1, never reused; it rides the entry
 * between the undo and redo stacks).  peek returns the serial of the
 * transaction the matching replay call would replay next, 0 when
 * that stack is empty.  The serial is an opaque identity token --
 * observers may associate their own state with it (the editor keys
 * caret-set snapshots this way) without the history learning what
 * that state means. */
lk_u32 lk_history_peek_undo_serial(const lk_edit_history *h);
lk_u32 lk_history_peek_redo_serial(const lk_edit_history *h);
