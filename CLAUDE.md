...
- **lk-sdl.c** — SDL3 integration: text measurement via SDL_ttf, event translation (`sdl_to_lk_event`), render list consumption, text texture caching (open-addressing cache keyed by `str_id`, default 2048 slots, configurable via `lk_window_cfg.text_cache_cap`). Cache uses frame-stamped LRU eviction: on miss, probes for an empty slot or match while tracking the stalest entry; if the table is full along the probe chain, evicts the stalest entry. Cache owns all textures (no uncached returns). Stats tracked: hits, misses, evictions, probe steps. One-time `SDL_Log` warning on long probe chains (>32).
