...
- **lcl-lk-test.c** (`test/lcl-lk-test.c`) — 67 headless tests via `lcl_eval_string`. Covers all non-SDL procs including text_input kind and overlay push/pop (modal defaults, focus trap, routed ESC), plus SDL-gated error-path tests (`lk::register_font` arity/type) that don't need a display. Includes the Layer-2 DSL harness (15 tests, DSL v2 stage 1): evals `lib/lk-dsl.lcl` via the `TEST_DSL_PATH` compile definition, drives the DSL's module state (`lk_dsl::_ui`/`lk_dsl::_tree`) from script, and asserts tree shape through the C API (`lk_tree_find_by_id`, `lk_node_kind_get`, prop getters, `lk_tree_get_presentation`, `lk_tree_has_tag`). These tests are the behavioral spec for the stage-3 syntax migration; two of them pin known warts (unknown flag ignored, trailing flag becomes boolean 1) that stage 3 must change.
