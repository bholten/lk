...
static void test_ctrl_click_translator_beats_add_caret(void) {
  pres_fix f;
  lk_u32 a;

  BEGIN_TEST("present: ctrl+click translator outranks add-caret");

  pfix_init(&f, "hello world", 0, 1);

  /* candidate over [0,5) with a ctrl+primary translator */
  a = lk_annot_add(f.store, 0, 5, "l", NULL, NULL, 0);
  lk_annot_set_present(f.store, a, intern(&f, "act"), lk_v_i32(1));
...
void lk_present_run_tests(void) {
  ...
  test_ctrl_click_translator_beats_add_caret();
